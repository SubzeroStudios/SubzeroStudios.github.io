...
function Sprite(scene, imageFile, width, height){
  ...
  this.distanceTo = function(sprite){
    //calculates the distance between two sprites in pixels
    //get centers
    ...
    //get deltas
    deltaX = this.centerX - sprite.centerX;
    deltaY = this.centerY - sprite.centerY;
...
function Sound(name){
  //sound effect class for cross-platform sound
  //builds a sound effect based on a file name base
  //expects <name>.ogg and <name>.mp3 to be available
  
  console.log("building sound");
  this.snd = document.createElement("audio");
  //uncomment next line to view audio controls for testing
  ...
  src1.src = name + ".ogg";
  this.snd.appendChild(src1);
  
  src2 = document.createElement("source");
  src2.src = name + ".mp3";
  this.snd.appendChild(src2);
  ...
  this.loop = function(){
    ...
    this.snd.setAttribute("autoplay", "true");
    this.snd.setAttribute("loop", "true");
    this.snd.play();
  } // end loop
